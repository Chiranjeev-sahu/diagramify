 ...
 * which domains can access your API from the browser.
 *
 * Why do we need this?
 * - Your frontend (React) runs on http://localhost:5173
 * - Your backend (Express) runs on http://localhost:5000
...
const corsOptions = {
  /**
   * origin: Which domains are allowed to access your API
   *
   * Options:
   * - String: 'http://localhost:5173' (single domain)
   * - Array: ['http://localhost:5173', 'https://myapp.com'] (multiple domains)
   * - Function: Dynamic check (shown below)
   * - true: Allow ALL origins (DANGEROUS in production!)
   */
