...
import { Bot, Sparkles, User, Paperclip } from "lucide-react";
...
export default function AIChat() {
  ...
  const { currentDiagram, repromptDiagram, isGenerating, getDiagramById } =
    useDiagramStore();
  ...
  const handleViewVersion = async (diagramId) => {
    if (!diagramId) return;
    await getDiagramById(diagramId);
  };
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center bg-white p-8 text-center">
        ...
        <p className="text-sm text-slate-500 max-w-sm">
          Select a diagram from the sidebar or create a new one to start
          chatting with AI
        </p>
      </div>
    );
                ...
                {msg.resultingDiagramId && (
                  <div className="mt-3 pt-3 border-t border-slate-300">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          handleViewVersion(msg.resultingDiagramId)
                        }
      ...
      <div className="p-4 border-t border-slate-200">
        <form onSubmit={handleSendMessage} className="flex gap-2">
          ...
          <button
            type="submit"
            disabled={isGenerating}
            className="px-6 py-3 rounded-2xl bg-blue-600 text-white hover:bg-blue-700 transition-all shadow-lg shadow-blue-900/30 active:scale-95 disabled:opacity-50 flex items-center gap-2 font-bold text-sm border border-blue-500/20"
          >
            {isGenerating ? <Spinner size="sm" /> : "Update"}
          </button>
        </form>
      </div>
    </div>
